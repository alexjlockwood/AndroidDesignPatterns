...
Perhaps the best way to understand how the Transition Framework works is through an example. Consider the steps involved in running a simple `Fade` transition (**TODO: concrete example/video?**):

1. The developer calls `TransitionManager.beginDelayedTransition(ViewGroup, Transition)`, passing the scene root and a `Fade` transition as the arguments. The framework immediately calls the transition's `captureStartValues(TransitionValues)` method for each `view` in the scene, and the `Fade` transition responds by recording each `view`'s visibility in the `TransitionValues` argument.
2. Immediately after `beginDelayedTransition()` returns, the developer sets each view in the scene to `INVISIBLE`.
3. On the next display frame, the framework calls the transition's `captureEndValues(TransitionValues)` method for each `view` in the scene. The `Fade` transition records each `view`'s (recently updated) visibility in the `TransitionValues` argument.
4. The framework calls the transition's `createAnimator(ViewGroup, TransitionValues, TransitionValues)` method. For each view that started as `VISIBLE` and ended as `INVISIBLE`, the `Fade` transition creates an `ObjectAnimator` that will animate the view's `alpha` property to `0`. An `AnimatorSet` containing all of the resulting `ObjectAnimator`s is returned back to the framework.
5. The framework runs the returned `Animator` and all views gradually fade out of the scene.

There are two imporant points. The first is that the developer no longer needs to worry about animators (the transition abstracts the idea of animators from the developer). The developer just needs to set the start and end values and pick a transition that can translate the differences into an animator. The second is that it becomes very easy to swap out different transition objects to alter the resulting animation. For example, we could swap out the `Fade` for an `Explode` or a `Slide`. We could even create our own custom transition that implements `createAnimator()` differently to obtain our own custom effect. As we will see in the next section, these points make `Transition`s prime candidates for being the core of the Activity Transitions.
