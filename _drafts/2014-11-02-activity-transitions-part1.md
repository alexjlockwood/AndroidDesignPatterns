...
There are two imporant points to be made here. The first is that the developer no longer needs to worry about animators (the transition abstracts the idea of animators from the developer). The developer just needs to set the start and end values and pick a transition that can translate the differences into an animator. The second is that it becomes very easy to swap out different transition objects to alter the resulting animation. For example, we could swap out the `Fade` for an `Explode` or a `Slide`. We could even create our own custom transition that implements `createAnimator()` differently to obtain our own custom effect. As we will see in the next section, these points make `Transition`s prime candidates for being the core of the Activity Transitions. (**TODO: at some point we should mention that the framework does a lot of the work involved in starting/setting up transitions, but it is very important to know how things work under the hood for a complete understanding**)
